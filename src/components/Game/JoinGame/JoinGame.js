...
function JoinGame() {
  ...
  const result = (message, playerId, gameId) => {
    if (message === "correct") {
      ...
      alert("Bạn đã đưa ra một pin xấu hoặc trò chơi không tồn tại")
    }
  }
      ...
      {!isPlayerAdded ? (
        <div className={styles.section}>
          ...
          <input
            ...
            ref={pinRef}
            placeholder={
              isLanguageEnglish ? "Write here a pin" : "Nhập mã PIN vào đây"
            }
          />
        ...
        <div className={styles.section}>
          ...
          <h4>
            {isLanguageEnglish ? "Waiting on a host to start the game" : "Đợi chủ nhà bắt đầu trò chơi"}
          </h4>
          <CircularProgress />
        </div>
      )}
